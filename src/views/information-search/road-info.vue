...
var vectorLayer;
const layerNameEncode = 'Test%3Aroad_line'
//地图初始化
const view = new View({
...
export default {
  ...
  methods: {
    ...
    getRoadGeoJson: async function (vueObject) {
      try {
        let url = process.env.VUE_APP_GIS_BASE_URL + '/Test/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=' + layerNameEncode + '&maxFeatures=500000000&outputFormat=application%2Fjson';
    ...
    roadTypeChange () {
      ...
      var checkStyleArray = [];
      for (var j = 0; j < roadTypeColor.length; j++) {
        let checkedStyle = new Style({
          stroke: new Stroke({
            color: roadTypeColor[j],
            width: 6,
          }),
        });
        checkStyleArray.push(checkedStyle);
      }

      let uncheckStyle = new Style({
        stroke: new Stroke({
          width: 0,
        }),
      });
      for (var i = 0; i < features.length; i++) {
        var featureAttr = features[i].get("layer");
        if (!featureAttr) {
          continue;
        }
        var layerNameIndex = roadTypeName.findIndex(item => {
          return item == featureAttr;
        });
        if (layerNameIndex == -1) {
          continue;
        }
        let checkedRoadTypeVal = checkedRoadType.find(item => {
          return item == layerNameIndex + 1;
        });
        if (checkedRoadTypeVal) {
          features[i].setStyle(checkStyleArray[layerNameIndex]);
        } else {
